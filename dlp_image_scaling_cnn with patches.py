...
class SRPatchDataset(Dataset):
    def __init__(self, hr_dir, patch_size=33, scale=3, patches_per_image=3):
        self.hr_dir = hr_dir
    ...
    def __getitem__(self, idx):
        ...
        if w < ps or h < ps:
            hr_img = resize(hr_img, (max(h, ps), max(w, ps)))

        x = random.randint(0, hr_img.width - ps)
        y = random.randint(0, hr_img.height - ps)
        hr_patch = hr_img.crop((x, y, x + ps, y + ps))

        lr_size = ps // self.scale
        lr_patch = hr_patch.resize((lr_size, lr_size), Image.BICUBIC)
        lr_patch = lr_patch.resize((ps, ps), Image.BICUBIC)
...
dataset = SRPatchDataset(hr_dir=hr_dir, patch_size=33, scale=3, patches_per_image=3)
...
# save the model
torch.save(model.state_dict(), "srcnn_trained_on_patches_rgb_9x5x5.pth")

#load the model
# model = SRCNN().to(device) 
# model.load_state_dict(torch.load("srcnn_rgb_9x5x5.pth"))
...
plt.savefig("sr_patched_comparison.png", bbox_inches="tight", dpi=300)
plt.show()
...
# model.eval()
# with torch.no_grad():
#     for i, (lr_patch, hr_patch) in enumerate(test_loader):
#         lr_patch = lr_patch.to(device)
#         hr_patch = hr_patch.to(device)

#         sr_patch = model(lr_patch).clamp(0, 1)

#         # Combine into one image: LR | SR | HR
#         combined = torch.cat([lr_patch, sr_patch, hr_patch], dim=3)  # concatenate width-wise

#         save_path = os.path.join(output_dir, f"patch_compare_{i:04d}.png")
#         save_image(combined.squeeze(0), save_path)

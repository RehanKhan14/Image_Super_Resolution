...
class DSRCNN(nn.Module):
    def __init__(self):
        super(DSRCNN, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=9, padding=4)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=5, padding=2)

        self.deconv1 = nn.ConvTranspose2d(64, 64, kernel_size=5, padding=2)
        self.deconv2 = nn.ConvTranspose2d(64, 64, kernel_size=9, padding=4)
...
# save the model
torch.save(model.state_dict(), "dsrcnn_rgb 20 epochs 3x scale.pth")
...
plt.subplot(1,3,2); show_image(sr_img, "DSRCNN Output")
plt.subplot(1,3,3); show_image(hr_img, "Ground Truth")
plt.savefig("dsr_comparison.png", bbox_inches="tight", dpi=300)
plt.show()

import os

output_dir = "./dsrcnn_outputs"
os.makedirs(output_dir, exist_ok=True)
...
with torch.no_grad():
    for i, (lr_img, hr_img, fname) in enumerate(test_loader):
        ...
        # Save output
        save_path = os.path.join(output_dir, f"dsr_{base_name}.png")
